...
1. 从览沃 GitHub 获取览沃 ROS 驱动程序
   ...
   注意：务必使用上面的命令克隆代码到本地，否则会因为文件路径的问题而编译出错
...
注意：上图中 X ，在 MID-100_Left/MID-40/Horizon/Tele 产品中对应为 1 ，在 MID-100_Middle 中对应为 2，在MID-100_Right 中对应为 3 。
...
1.1 览沃 pointcloud2(PointXYZRTL) 点云格式，详细如下 : 
...
1.3 PCL 库中标准 pointcloud2(pcl::PointXYZI) 点云格式 :
...
| extrinsic_parameter_source | 整型   | 是否使能外参自动补偿<br/>0 -- 不补偿 LiDAR 外参<br/>1 -- 自动补偿 LiDAR 外参<br/>说明：连接多个 LiDAR 时，如果要使用外参自动补偿功能，必需先使用 livox viewer 标定好外参并保存到 LiDAR 中 | 0               |
